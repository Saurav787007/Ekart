pipeline {
    ...
    stages {
        stage('git checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/Saurav787007/Ekart.git'
            }
        }
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t saurav787007/ekart:latest -f docker/Dockerfile ."
                    }
            }
        }
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   ...
                   sh 'docker login -u saurav787007 -p ${dockerhub.pwd}'}
                   sh 'docker push saurav787007/ekart:latest'
                }
            }
        }
